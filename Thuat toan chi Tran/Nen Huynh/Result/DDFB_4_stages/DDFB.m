function result = DDFB(xx,b_num,order)
% name: ddfb_4_stages
% xx is input image
% b_num is the branch or stage number i.e. 1-4
...
switch b_num
    case 1
        ...
        f1=imfilter(xx,f_filter1,'circular','same');
        f2=imfilter(xx,f_filter2,'circular','same');
        clear('f_filter1','f_filter2');
%         result = cell(1,2);
%         result{1} = f1;
%         result{2} = f2;
        result = zeros(order,order,2*b_num);
        result(:,:,1) = f1;
        result(:,:,2) = f2;
    case 2
        f_filter1 =hour_low(order);
